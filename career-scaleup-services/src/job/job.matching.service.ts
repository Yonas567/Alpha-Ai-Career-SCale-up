...
export class JobMatchingService {
  ...
  private getOverlapTokens(a: Set<string>, b: Set<string>): string[] {
    const matches: string[] = [];
    for (const token of a) if (b.has(token)) matches.push(token);
    return matches;
  }
  ...
  getMatchScore(profile: User, job: Omit<JobDto, 'recruiterId'>) {
    const profileTokens = this.getProfileTokens(profile);
    const jobTokens = this.getJobTokens(job);
    ...
    const jobSkillTokens = new Set([
      ...jobTokens.qualifications,
      ...jobTokens.responsibilities,
      ...jobTokens.description,
    ]);
    ...
    return {
      score,
      matches: {
        profession: this.getOverlapTokens(profileTokens.profession, jobTokens.title),
        skills: this.getOverlapTokens(profileTokens.skills, jobSkillTokens),
        education: this.getOverlapTokens(
          profileTokens.education,
          jobTokens.qualifications,
        ),
        summary: this.getOverlapTokens(
          profileTokens.summary,
          jobTokens.description,
        ),
      },
